...
class SdkPayment
{
	...
	function buildRequestHttp() {
		...
		//待请求参数数组字符串
		$request_data = $this->buildRequestPara($parameter);

		//远程获取数据
		$sResult = $this->getHttpResponsePOST($this->alipay_gateway_new, $this->cacert,$request_data,trim(strtolower($this->_input_charset)));
		return $sResult;
	}	
}
